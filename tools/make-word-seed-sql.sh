...
curl -s $URL > words.txt

# filter words
cat words.txt | sed "s/.*/\L&/" | grep -E "^[a-z]{1,12}$" | sort | uniq > words_filtered.txt

# display words analytics
cat words_filtered.txt | awk '{print length($0)}' | sort -n | uniq -c |
awk '
...
sed "s/.*/('&'),/" words_filtered.txt | sed '$s/,/;/' >> seed-words.sql
