...
public class Game {
	...
	private static void showMap() {
		...
		mapPanel.setRenderer(new ComponentRenderer<Panel>() {
			...
			public void drawComponent(TextGUIGraphics graphics, Panel component) {
				...
				goldLabel.setText("Gold: " + GameMap.getGoldCount());
			}
	...
	private static void showCombat() {
		...
		Panel scoresPanel = new Panel(new GridLayout(2));
			...
			public void drawComponent(TextGUIGraphics graphics, Panel component) {
				...
				synchronized (GameCombat.getSwimmingFishes()) {
					for (GameCombat.ActiveFish fish : GameCombat.getSwimmingFishes()) {
						...
						// print head
						graphics.setForegroundColor(fish.type.color).putString(fish.x, row, fish.type.head);
						...
						// print tail
						graphics.setForegroundColor(fish.type.color).putString(wordX + fish.word.length(), row, fish.type.tail);
					}
				}
			}
		...
		window.addWindowListener(new WindowListenerAdapter() {
			...
			public void onInput(Window basePane, KeyStroke keyStroke, AtomicBoolean deliverEvent) {
				...
				if (keyType == KeyType.Escape) {
					...
					MessageDialogButton result = MessageDialog.showMessageDialog(gui, "Confirm Exit", "Return to Menu? (This will remove all progress from this Combat session!)", MessageDialogButton.No, MessageDialogButton.Yes);
					if (result == MessageDialogButton.Yes) {
						window.close();
					} else {
		...
		panel.addComponent(scoresPanel);
