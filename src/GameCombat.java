...
public class GameCombat {
	...
	private static int MIN_FISH_GAP = 2;
	private static int FULLSCREEN_WIDTH = STICKMAN_COL + STICKMAN_WIDTH + LANE_WIDTH + RIGHT_MARGIN;

	private static float STARTING_FISH_SPEED = 0.5f; // secs per char
	...
	private static long pausedTime, pauseEnd, pauseStart;
	...
	public static void resetCombat() {
		swimmingFishes = new CopyOnWriteArrayList<>();
	...
	public static void updateWave() {
		if (remainingInWave <= 0 && swimmingFishes.isEmpty()) {
			...
			wave++;
			remainingInWave = wave * 25;
			secPerChar = Math.max(0.1, STARTING_FISH_SPEED - ((wave - 1) * 0.25));
		}
	}

	public void handleCharacterInput(int userID, char typed) {
		Iterator<ActiveFish> it = swimmingFishes.iterator();
		while (it.hasNext()) {
			ActiveFish f = it.next();
			if (f.word.charAt(f.lettersTyped) == typed) {
				// if you typed the next letter
				if (f.lettersTyped + 1 == f.word.length()) {
	...
	private static boolean showExitPrompt(Screen screen, TextGraphics canvas, int width) throws Exception {
		while (true) {
			canvas.setForegroundColor(TextColor.ANSI.CYAN);
			canvas.putString(width / 2 - 15, 10, "	  PAUSED	  ", SGR.REVERSE);
			canvas.setForegroundColor(TextColor.ANSI.WHITE);
			canvas.putString(width / 2 - 15, 11, " [C]ONTINUE COMBAT ", SGR.BOLD);
			canvas.putString(width / 2 - 15, 12, " [Q]UIT (LOSE PROGRESS) ");
			screen.refresh();
